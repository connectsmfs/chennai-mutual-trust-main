...
const Calculators = () => {
  ...
  const [stepUpSipRate, setStepUpSipRate] = useState(12);
  const [stepUpSipYears, setStepUpSipYears] = useState(10);
  ...
  const calculateStepUpSIP = () => {
    const monthlyRate = stepUpSipRate / 12 / 100;
    let totalInvested = 0;
    let futureValue = 0;
    let currentMonthlyAmount = stepUpSipAmount;

    // Determine when to apply step-up based on frequency
    const stepUpMonths = stepUpFrequency === "monthly" ? 1 : 
                         stepUpFrequency === "quarterly" ? 3 : 
                         stepUpFrequency === "half-yearly" ? 6 : 12;

    for (let month = 1; month <= stepUpSipYears * 12; month++) {
      totalInvested += currentMonthlyAmount;
      const monthsRemaining = (stepUpSipYears * 12) - month;
      futureValue += currentMonthlyAmount * Math.pow(1 + monthlyRate, monthsRemaining);

      // Apply step-up at the specified frequency
      if (month % stepUpMonths === 0 && month < stepUpSipYears * 12) {
        if (stepUpType === "percentage") {
          currentMonthlyAmount = currentMonthlyAmount * (1 + stepUpPercentage / 100);
        } else {
          currentMonthlyAmount = currentMonthlyAmount + stepUpPercentage;
        }
      }
    }
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-5xl mx-auto">
          <Tabs defaultValue="sip" className="w-full">
            <TabsList
  className="
    grid w-full grid-cols-3 lg:grid-cols-7 
    justify-center place-items-center text-center
    h-auto p-2 bg-card rounded-2xl shadow-lg gap-2
  "
>
            ...
            <TabsContent value="stepup-sip" className="mt-8">
              <Card className="border-0 shadow-2xl rounded-3xl overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-primary to-accent text-primary-foreground p-8">
                  <CardTitle className="text-3xl">Step-up SIP Calculator</CardTitle>
                  <CardDescription className="text-primary-foreground/90 text-base">
                    Calculate returns on your SIP with annual step-up increases
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-8">
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                      <div>
                        <Label htmlFor="stepUpSipAmount" className="text-base font-semibold mb-3 block">
                          Initial Monthly Investment
                        </Label>
                        <Input
                          id="stepUpSipAmount"
                          type="number"
                          value={stepUpSipAmount}
                          onChange={(e) => setStepUpSipAmount(Number(e.target.value))}
                          className="h-12 text-lg rounded-xl"
                        />
                      </div>
                      <div>
                        <Label htmlFor="stepUpType" className="text-base font-semibold mb-3 block">
                          Step-up Type
                        </Label>
                        <Select value={stepUpType} onValueChange={(value: "percentage" | "value") => setStepUpType(value)}>
                      ...
                      <div>
                        <Label htmlFor="stepUpPercentage" className="text-base font-semibold mb-3 block">
                          {stepUpType === "percentage" ? "Step-up (%)" : "Step-up Amount (₹)"}
                        </Label>
                        <Input
                          id="stepUpPercentage"
                          type="number"
                      ...
                      <div>
                        <Label htmlFor="stepUpFrequency" className="text-base font-semibold mb-3 block">
                          Step-up Frequency
                        </Label>
                        <Select value={stepUpFrequency} onValueChange={(value: "monthly" | "quarterly" | "half-yearly" | "annually") => setStepUpFrequency(value)}>
                          <SelectTrigger className="h-12 text-lg rounded-xl">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div>
                        <Label htmlFor="stepUpSipRate" className="text-base font-semibold mb-3 block">
                          Expected Annual Return (%)
                        </Label>
                        <Input
                          id="stepUpSipRate"
                          type="number"
                          value={stepUpSipRate}
                          onChange={(e) => setStepUpSipRate(Number(e.target.value))}
                          className="h-12 text-lg rounded-xl"
                        />
                      </div>
                      <div>
                        <Label htmlFor="stepUpSipYears" className="text-base font-semibold mb-3 block">
                          Investment Period (Years)
                        </Label>
                        <Input
                          id="stepUpSipYears"
                          type="number"
                          value={stepUpSipYears}
                          onChange={(e) => setStepUpSipYears(Number(e.target.value))}
                          className="h-12 text-lg rounded-xl"
                        />
                      </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-primary/5 to-accent/5 rounded-2xl p-8 space-y-6">
                      ...
                      <div className="h-64 mb-6">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={[
                                { name: "Invested amount", value: stepUpSipResult.invested },
                                { name: "Est. returns", value: stepUpSipResult.returns }
                              ]}
                              cx="50%"
                              cy="50%"
                              innerRadius={60}
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center pb-4 border-b border-border">
                          <span className="text-muted-foreground">Total Investment</span>
                          <span className="text-xl font-bold">{formatCurrency(stepUpSipResult.invested)}</span>
                        </div>
                        <div className="flex justify-between items-center pb-4 border-b border-border">
                          <span className="text-muted-foreground">Expected Returns</span>
                          <span className="text-xl font-bold text-green-600">{formatCurrency(stepUpSipResult.returns)}</span>
                        </div>
                        <div className="flex justify-between items-center pt-2">
                          <span className="text-lg font-semibold">Maturity Value</span>
                          <span className="text-3xl font-bold text-primary">{formatCurrency(stepUpSipResult.futureValue)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
